...
// Example: public interface DoctorRepository extends JpaRepository<Doctor, Long> {}
    ...
    @Query("SELECT doc FROM Doctor doc " +
            "WHERE doc.doctor.name LIKE LOWER(CONCAT('%', :name, '%'))")
    ...
    @Query("SELECT doc FROM Doctor doc " +
            "WHERE doc.doctor.name LIKE LOWER(CONCAT('%', :name, '%')) " +
            "AND doc.doctor.speciylity LIKE LOWER(CONCAT('%', :specialty, '%'))")
    ...
    @Query("SELECT doc FROM Doctor doc " +
            "WHERE doc.doctor.speciylity LIKE LOWER(CONCAT('%', :specialty, '%'))")
